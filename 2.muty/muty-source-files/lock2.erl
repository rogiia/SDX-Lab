...
start(MyId) -> spawn(fun () -> init(MyId) end).

init(MyId) ->
    receive
      {peers, Nodes} -> open(Nodes, MyId);
      stop -> ok
    end.
    ...
    receive
      {take, Master, Ref} ->
        ...
        wait(Nodes, Master, Refs, [], Ref, MyId);
          {request, From, Ref, _} ->
        From ! {ok, Ref}, open(Nodes, MyId);
          stop -> ok
    end.

requests(Nodes, MyId) ->
    lists:map(fun (P) ->
		      R = make_ref(), P ! {request, self(), R, MyId}, R
	      end,
	      Nodes).

wait(Nodes, Master, [], Waiting, TakeRef, MyId) ->
    Master ! {taken, TakeRef}, held(Nodes, Waiting, MyId);
wait(Nodes, Master, Refs, Waiting, TakeRef, MyId) ->
    receive
      {request, From, Ref, LockID} ->
        ...
        wait(Nodes, Master, [NewRef | Refs], Waiting, TakeRef,
              MyId);
          true ->
      ...
      {ok, Ref} ->
        ...
        wait(Nodes, Master, NewRefs, Waiting, TakeRef, MyId);
          release -> ok(Waiting), open(Nodes, MyId)
    end.
