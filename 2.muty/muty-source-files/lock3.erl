...
requests(Nodes, MyId, Clock) ->
    lists:map(fun (P) ->
		      R = make_ref(), P ! {request, self(), R, MyId, Clock}, R
	      end,
	      Nodes).

wait(Nodes, Master, [], Waiting, TakeRef, MyId, _,
     NextClock) ->
    ...
    receive
      ...
      {ok, Ref} ->
        ...
        wait(Nodes, Master, NewRefs, Waiting, TakeRef, MyId,
            Clock, NextClock);
      release -> ok(Waiting), open(Nodes, MyId, NextClock)
    end.
...
held(Nodes, Waiting, MyId, Clock) ->
    receive
      ...
      release -> ok(Waiting), open(Nodes, MyId, Clock)
    end.
